...
void ParentEngine::run() {
	...
	if (astParam2->getParameterType() == VT_CONSTANTINTEGER || astParam2->getParameterType() == VT_CONSTANTSTRING) {
	...
	for (iter = first.begin();  iter != first.end(); iter++) { // for every statement find the modified value
		...
		vector<int> childList = pkbManager->getParent()->getChild(*iter, false);
