	...
	for (vector<string>::size_type counter = 0; counter < returnResults.size(); counter++){
		...
		if (returnIndividualResult.find(".") != string::npos){
			...
			if (strcmpi(returnResultRawType.c_str(), "variable") == 0 && strcmpi(returnResultExtension.c_str(), "varName") != 0){
				...
				return false;
			}else if ((strcmpi(returnResultRawType.c_str(), "while") == 0 || strcmpi(returnResultRawType.c_str(), "if") == 0 || strcmpi(returnResultRawType.c_str(), "procedure") == 0) && strcmpi(returnResultExtension.c_str(), "stmtLst") != 0){
				cout << "stmt not stmt#" << endl;
				return false;
			}else{
